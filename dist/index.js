...
// 获取命令行参数
const command = (_a = process.argv[2]) === null || _a === void 0 ? void 0 : _a.toLowerCase();
const query = process.argv[3];
if (!command) {
    // 检查是否使用了--sse命令行参数
    if (config_1.default.sse.enabled) {
        console.log('正在启动 SSE 服务器模式...');
        (0, server_1.startSseServer)().catch(error => {
    ...
    console.log(`正在启动 SSE 服务器，端口: ${config_1.default.sse.port}...`);
    (0, server_1.startSseServer)().catch(error => {
        console.error('SSE 服务器启动失败:', error);
        process.exit(1);
    });
    ...
    // 处理命令
    handleCommand(command, query).catch(async (error) => {
...
// 处理命令的函数
async function handleCommand(command, query) {
    try {
        switch (command) {
            case 'brave': {
                const result = await (0, mcp_1.performWebSearch)(query);
                console.log(result);
                break;
            }
            case 'fetch': {
                const html = await (0, mcp_1.fetchHTML)(query);
                console.log(html);
                break;
            }
            case 'content': {
                const content = await (0, mcp_1.extractContent)(query);
                console.log(content);
                break;
            }
            case 'bing': {
                const result = await (0, mcp_1.searchBing)(query);
                console.log(result);
                break;
            }
            case 'google': {
                const result = await (0, mcp_1.searchGoogle)(query);
                console.log(result);
                break;
            }
            case 'baidu': {
                const result = await (0, mcp_1.searchBaidu)(query);
                console.log(result);
                break;
            }
            case '360': {
                const result = await (0, mcp_1.search360)(query);
                console.log(result);
                break;
            }
            case 'browse': {
                const content = await (0, mcp_1.browsePage)(query);
                console.log(content);
                break;
            }
            case 'startmcp': {
