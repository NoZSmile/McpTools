...
if (PROXY_URL) {
  ...
  axiosConfig.httpAgent = new ProxyAgent(PROXY_URL as any);
  axiosConfig.httpsAgent = new ProxyAgent(PROXY_URL as any);
...
async function getPoisData(ids: string[]): Promise<BravePoiResponse> {
  ...
  const response = await axiosClient.get(url.toString());
  const poisResponse = response.data as BravePoiResponse;
  return poisResponse;
}
...
async function getDescriptionsData(ids: string[]): Promise<BraveDescription> {
  ...
  ids.filter(Boolean).forEach(id => url.searchParams.append('ids', id));
  const response = await axiosClient.get(url.toString());
  const descriptionsData = response.data as BraveDescription;
